...
DoublyLinkedListNode* reverse(DoublyLinkedListNode* llist) {
    // Will store the new LinkedList
    DoublyLinkedListNode* new_llist = NULL;
    // Will point the current element
    DoublyLinkedListNode* cur = NULL;
    // While is not NULL
    while (llist) {
        // CUR = Create a new Node with the current node info
        cur = create_doubly_linked_list_node(llist->data);
        // CUR->NEXT will point to the NEWLL Head
        cur->next = new_llist;
        // NEWLL->PREV will point to ne new CUR node
        if (new_llist) {
            new_llist->prev = cur;
        }
        // The NEW LinkedList Head is CUR
        new_llist = cur;
        // Move to the next element in the LinkedList
        llist = llist->next;
    }
