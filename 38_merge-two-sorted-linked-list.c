...
SinglyLinkedListNode* mergeLists(SinglyLinkedListNode* head1, SinglyLinkedListNode* head2) {
    // Create a new LinkedList which will have all the elements merged
    SinglyLinkedListNode* merged = NULL ;
    // Assign the first element to create the head
    if (head1->data <= head2->data) {
        SinglyLinkedListNode* new_node = create_singly_linked_list_node(head1->data);
        merged = new_node;
        head1 = head1->next ;
    }
    else {
        SinglyLinkedListNode* new_node = create_singly_linked_list_node(head2->data);
        merged = new_node;
        head2 = head2->next;
    }
    // Points to the head of the merged list
    SinglyLinkedListNode* final_list = merged ;
    // While none is NULL
    while (head1 && head2){
        if (head1->data <= head2->data){
            SinglyLinkedListNode* new_node = create_singly_linked_list_node(head1->data);
    ...
    // Add the rest of the elements of the remaining list
    if (head1){
        while (head1){
            SinglyLinkedListNode* new_node = create_singly_linked_list_node(head1->data);
            merged->next = new_node ;
